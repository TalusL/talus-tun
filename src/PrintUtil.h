//
// Created by liangzhuohua on 2023/1/13.
//

#ifndef TALUSTUN_PRINTUTIL_H
#define TALUSTUN_PRINTUTIL_H
...
inline void hex(char* source, char* dest, ssize_t count)
{
...
inline int has_ports(int protocol)
{
    switch(protocol) {
        case IPPROTO_UDP:
...
inline void dump_ports(int protocol, int count, const char* buffer) {
...
inline void dump_packet_ipv4(int count, char* buffer)
{
    if (count < 20) {
...
inline void dump_packet_ipv6(int count, char* buffer)
{
    if (count < 40) {
...
inline void dump_packet(int count, char* buffer)
{
