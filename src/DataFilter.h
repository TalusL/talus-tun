...
public:
    static void Filter(const Buffer::Ptr &buf){
        ...
        static auto *hp = reinterpret_cast<const uint64_t *>(passMd5.data());
        static auto *lp = reinterpret_cast<const uint64_t *>(passMd5.data() + sizeof(uint64_t));
        uint64_t pass = buf->size()%2?*hp:*lp;
